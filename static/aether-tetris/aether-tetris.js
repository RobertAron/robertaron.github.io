...
function findWasmBinary() {
  return locateFile('aether-tetris.wasm');
}
...
var ASM_CONSTS = {
  1221800: () => { document.getElementById('canvas').style.width = '100%'; },  
 1221860: () => { document.getElementById('canvas').style.height = '100%'; },  
 1221921: () => { document.getElementById('canvas').style.aspectRatio = '2 / 1'; },  
 1221988: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 1222049: () => { document.getElementById('canvas').style.cursor = 'auto'; },  
 1222110: ($0) => { stringToUTF8(window.location.href, $0, 256) }
};
function _ae_performance_now() { return performance.now(); }
function _ae_FileSystem_ReadImpl(url,arg,timeoutMs) { var handled = false; var xhr = new XMLHttpRequest(); xhr.timeout = timeoutMs; xhr.open('GET', UTF8ToString(url), true); xhr.responseType = 'arraybuffer'; xhr.ontimeout = function xhr_ontimeout() { if (!handled) { handled = true; __ae_FileSystem_ReadFail(arg, xhr.status, true); } }; xhr.onload = function xhr_onload() { if (!handled) { handled = true; if(xhr.status != 200) { __ae_FileSystem_ReadFail(arg, xhr.status, false); return; } if(!xhr.response) { __ae_FileSystem_ReadSuccess(arg, 0, 0); return; } var byteArray = new Uint8Array(xhr.response); var buffer = _malloc(byteArray.length); if(!buffer) { __ae_FileSystem_ReadFail(arg, 0, false); return; } HEAPU8.set(byteArray, buffer); __ae_FileSystem_ReadSuccess(arg, buffer, byteArray.length); __ae_em_free(buffer); } }; xhr.onerror = function xhr_onerror() { if (!handled) { handled = true; __ae_FileSystem_ReadFail(arg, xhr.status || 0, false); } }; xhr.onabort = function xhr_onabort() { if (!handled) { handled = true; __ae_FileSystem_ReadFail(arg, 0, false); } }; xhr.onloadend = function xhr_onloadend() { if (!handled) { handled = true; __ae_FileSystem_ReadFail(arg, xhr.status || 0, false); } }; try { xhr.send(null); } catch(e) { if (!handled) { handled = true; __ae_FileSystem_ReadFail(arg, 0, false); } } }

// Imports from the Wasm binary.
var _main = Module['_main'] = makeInvalidEarlyAccess('_main');
var _malloc = makeInvalidEarlyAccess('_malloc');
var __ae_FileSystem_ReadSuccess = Module['__ae_FileSystem_ReadSuccess'] = makeInvalidEarlyAccess('__ae_FileSystem_ReadSuccess');
var __ae_FileSystem_ReadFail = Module['__ae_FileSystem_ReadFail'] = makeInvalidEarlyAccess('__ae_FileSystem_ReadFail');
var __ae_em_free = Module['__ae_em_free'] = makeInvalidEarlyAccess('__ae_em_free');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['_ae_FileSystem_ReadSuccess'] != 'undefined', 'missing Wasm export: _ae_FileSystem_ReadSuccess');
  assert(typeof wasmExports['_ae_FileSystem_ReadFail'] != 'undefined', 'missing Wasm export: _ae_FileSystem_ReadFail');
  assert(typeof wasmExports['_ae_em_free'] != 'undefined', 'missing Wasm export: _ae_em_free');
  ...
  __ae_FileSystem_ReadSuccess = Module['__ae_FileSystem_ReadSuccess'] = createExportWrapper('_ae_FileSystem_ReadSuccess', 3);
  __ae_FileSystem_ReadFail = Module['__ae_FileSystem_ReadFail'] = createExportWrapper('_ae_FileSystem_ReadFail', 3);
  __ae_em_free = Module['__ae_em_free'] = createExportWrapper('_ae_em_free', 1);
